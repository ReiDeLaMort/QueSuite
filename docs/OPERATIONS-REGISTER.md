...
This is a living discovery register. The capabilities below are proposals for consideration, not implemented features or a promise to build every module. The existing [roadmap](ROADMAP.md) remains the delivery sequence; [SHARED-WORK.md](SHARED-WORK.md) describes the proposed QA–Maintenance–Accounting workflow.
...
- Confirmed product direction, still to implement: both manual meter histories and operational/shift notes; shared departmental work including QA, Maintenance, and Accounting; role guidance and revisioned instructions.
...
All register entries start with **Decision: Captured / Delivery: Not scheduled / Accountable owner: Unassigned**. Department names are suggested participants, not assumptions about the company's organization chart. Examples are hypothetical. An individual owner and real example should be identified before an item enters delivery planning.
...
| OPS-005 | Parts and materials movements                       | Item, quantity/unit, storeroom/bin, reservation, issue to work, return, receipt, adjustment reason and count evidence                                  | Maintenance, Stores and Accounting can trace what was consumed and which work is waiting for material. Preserve transactions rather than only editing a stock total.                             |
| OPS-006 | Supplier and purchasing handoffs                    | Supplier/service reference, quote, request, approved commitment reference, promised date, receipt, shortage, return and service report                 | Connect waiting work to Purchasing, supplier follow-up, QA checks, and Accounting evidence. Receipt, authorization and payment remain distinct events.                                           |
